    ...
    <ScrollContainer>
      {recommendations.map((movie, index) => (
        <div key={index} style={itemStyle}>
          ...
          <button onClick={() => onMovieSelect(movie.title)} style={{ marginTop: '5px', fontSize: '0.7rem', width:'150px', height:'30px', whiteSpace:"nowrap", padding:'5px',fontFamily:'Noto sans KR',background: 'linear-gradient(0deg, #2a2f42 30%, #1c1e2c 70%)', color: '#f4f3f3',textOverflow: 'ellipsis'}}>
            {movie.title}
          </button>
        </div>
      ))}
